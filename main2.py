...
with st.form('population-form'):
    ...
    with col1:
        st.write("Chhoose a starting date")
    ...
    submit_btn = st.form_submit_button('Analyse', type = "primary")

# Logic to check input dates
...
if start_date not in df['Quarter'].tolist() or end_date not in df['Quarter'].tolist():
    st.error('No date available. Check your quarter and year selection.')
elif end_before_start(start_date, end_date) :
    st.error("Dates don't work. Start date must not be later than End date.")
else:
